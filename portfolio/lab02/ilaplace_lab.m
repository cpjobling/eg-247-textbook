...
% 
% In this lab exercise we will demonstrate the definition and use of rational 
% polynomials for the representation of Laplace transforms, the use of the |residue| 
% function to compute the residues of partial fraction expansions using a worked 
...
% to find the inverse Laplace transform for two typical non-trivial problems.
% 
% This next command opens the documentation for the |ilaplace| function. You 
% may wish to leave this open for consulation as you work through these exercises.

clearvars
doc ilaplace
%% Residues
% If you need to find the residues of a partial fraction expansion of a rational 
% polynomial
% 
% $$ F(s) = \frac{N(s)}{D(s)}$$
% 
...
plot(t,ft),ylim([-0.25,1.8])
%% The Inverse Laplace Function
% As illustrated above, the inverse laplace transform function is |ilaplace|

doc ilaplace
%% 
% If we use the symbolic toolbox, we do not need to compute the residues ourselves. 
% The solution is available straight from $F(s)$

Fs = (3*s + 2)/(s^2 + 3*s + 2);
ft = ilaplace(Fs)
fplot(ft,[0,4.5])
%% Factorising higher-order polynomials
% When computing Inverse Laplace Transforms of rational polynomials of order 
% 3 or higher by hand, it is useful to be able to factorize the denominator using 
% a computer.
% 
% Matlab provides two ways to do this using |roots| and |factor|.
% 
% The first is a numerical root finder, it needs the polynomial to have been 
% defined as a row vector of the numerical coefficients in descending powers of 
% s:
...
%% Problem 10
% use the function |collect| to expand |(s + 1)^3 * (s + 2)^2| to a polynomial. 
% Use this result and |residue| to verify the result of Problem 9.
...
% Transforms for your own selection of problems from each of Questions 1 to Question 
% 5 in the End of Chapter Exercises (Section 3.6 in the Text Book)